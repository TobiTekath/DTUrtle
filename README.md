...
While most RNA-seq data sets are mainly analysed in regard to
...
analysis in bulk or single-cell RNA-seq data, comparing the expression
proportions of a genes different transcript isoforms between conditions.
The results, a list of significantly differential genes and transcripts
isoforms, can be aggregated in an overview table and visualized in
multiple ways.

## Target audience

No in-depth knowledge about DTU or the statistical analysis behind it is
needed. If you have read data of an RNA-seq experiment at hand (or are
able to download one from a public repository), you are good to go.
Three exemplary vignettes for different species and sequencing
...
your species is needed (which is also already needed by the quantifying
tools).
