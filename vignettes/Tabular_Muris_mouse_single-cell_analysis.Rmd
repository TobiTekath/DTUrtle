...
```{r}
unfilt_gtf <- import_gtf("../gencode.vM24.annotation.gtf", feature_type = NULL, out_df = F)
...
cell_markers <- FindAllMarkers(tiss, only.pos=T)
```
...
interesting_dups <- interesting_dups[interesting_dups$`dup_markers$cluster`==T,]
...
DimPlot(tiss)
FeaturePlot(tiss, features = c("Rps24-202", "Rps24-210"), order = T)
```
